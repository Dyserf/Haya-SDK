...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.Haya = {}));
    ...
    const createState = (projectId, userConfig) => {
        var _a;
        const config = Object.assign(Object.assign(Object.assign({}, DEFAULT_CONFIG), userConfig), { autoTrack: Object.assign(Object.assign({}, DEFAULT_CONFIG.autoTrack), ((_a = userConfig.autoTrack) !== null && _a !== void 0 ? _a : {})) });
        return {
            projectId,
    ...
    const log = (...args) => {
        if (_debug)
            console.log("[Haya]", ...args);
    };
    const warn = (...args) => {
        if (_debug)
            console.warn("[Haya]", ...args);
    };
    const error = (...args) => {
        // Always log errors in debug mode; suppress in production
        if (_debug)
            console.error("[Haya]", ...args);
    };
    ...
    class EventBuffer {
        constructor(state) {
            ...
            this.snapshots = [];
            this.timer = null;
            this.retryDelays = [1000, 2000, 4000, 8000, 8000]; // exponential backoff
            this.state = state;
        }
        push(event) {
        ...
        start() {
            this.timer = setInterval(() => this.flush(), this.state.config.flushInterval);
        ...
        buildBatch() {
            if (this.events.length === 0 && this.snapshots.length === 0)
                return null;
            const batch = {
        ...
        flush(sync = false) {
            ...
            if (sync && navigator.sendBeacon) {
                ...
                const sent = navigator.sendBeacon(this.state.config.endpoint, new Blob([body], { type: "application/json" }));
                if (!sent)
        ...
        sendWithRetry(batch, attempt) {
            ...
            fetch(this.state.config.endpoint, {
                ...
                if (res.status === 429 || res.status >= 500) {
                    ...
                    warn(`Batch too large (413) — dropping ${batch.events.length} events and ${(_b = (_a = batch.snapshots) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0} snapshots`);
                }
                else if (!res.ok) {
        ...
        drainOfflineQueue() {
            try {
                ...
                fresh.forEach((entry) => this.sendWithRetry(entry.batch, 0));
    ...
    const isSensitiveElement = (el) => {
        ...
        if (!(el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement))
            return false;
        const type = (_b = (_a = el.type) === null || _a === void 0 ? void 0 : _a.toLowerCase()) !== null && _b !== void 0 ? _b : "";
        const name = ((_c = el.getAttribute("name")) !== null && _c !== void 0 ? _c : "").toLowerCase();
        const autocomplete = ((_d = el.getAttribute("autocomplete")) !== null && _d !== void 0 ? _d : "").toLowerCase();
    ...
    const initClickTracker = (state, push) => {
        const handler = (e) => {
            var _a, _b;
            ...
            if (state.config.ignoreSelectors.some((sel) => target.closest(sel)))
                return;
            push({
                type: "click",
                pageUrl: getPath(),
                ...
                payload: {
                    ...
                    text: isSensitiveElement(target)
                        ? "[masked]"
                        : (_b = (_a = target.innerText) === null || _a === void 0 ? void 0 : _a.slice(0, 100)) !== null && _b !== void 0 ? _b : "",
                },
            });
        };
    ...
    const initScrollTracker = (_state, push) => {
        let lastFired = 0;
        const handler = () => {
            const now = Date.now();
            if (now - lastFired < THROTTLE_MS$1)
                return;
            lastFired = now;
    ...
    const initMousemoveTracker = (_state, push) => {
        let lastFired = 0;
        const handler = (e) => {
            const now = Date.now();
     ...
     */
    const initRouteTracker = (_state, push) => {
        let currentPath = getPath();
        const firePageview = (url) => {
            if (url === currentPath)
                return;
            currentPath = url;
            push({
    ...
    const initFormTracker = (state, push) => {
        const onFocus = (e) => {
            var _a;
            const target = e.target;
            if (!isFormField(target))
                return;
            if (state.config.ignoreSelectors.some((s) => target.closest(s)))
                return;
            push({
                ...
                payload: {
                    action: "focus",
                    target: getSelector(target),
                    fieldType: (_a = target.type) !== null && _a !== void 0 ? _a : "unknown",
        ...
        const onBlur = (e) => {
            ...
            const value = isSensitiveElement(target)
                ? "[masked]"
                : ((_a = target.value) === null || _a === void 0 ? void 0 : _a.length)
                    ? `[${target.value.length} chars]`
                    : "[empty]";
            push({
                type: "form_interaction",
                pageUrl: getPath(),
                ...
                payload: {
                    action: "blur",
                    target: getSelector(target),
                    fieldType: (_b = target.type) !== null && _b !== void 0 ? _b : "unknown",
                    value,
                    masked: isSensitiveElement(target),
                },
            });
    ...
    const isFormField = (el) => el instanceof HTMLInputElement ||
     ...
     */
    const initReplayEngine = (state, pushSnapshot, pushEvent, flushNow) => {
        ...
        const finalize = (reason, sync = false) => {
            if (stopped)
                return;
            stopped = true;
            clearTimers();
        ...
        const ACTIVITY_EVENTS = ["mousemove", "mousedown", "click", "scroll", "keydown", "touchstart"];
     ...
     * });
     */
    const init = (projectId, userConfig = {}) => {
        if (isInitialized) {
        ...
        const push = (event) => buffer.push(event);
        ...
        if (state.config.sessionReplay) {
            teardowns.push(initReplayEngine(state, (snapshot) => buffer.pushSnapshot(snapshot), push, (sync) => buffer.flush(sync)));
        }
    ...
    const track = (eventName, properties = {}) => {
        ...
        buffer.push({
            ...
            pageUrl: getPath(),
            timestamp: Date.now(),
            payload: Object.assign({ name: eventName }, properties),
        });
    };
    /**
    ...
    exports.default = haya;
    exports.flush = flush;
    exports.ignore = ignore;
    exports.init = init;
    exports.mask = mask;
    exports.reset = reset;
    exports.setDebug = setDebugMode;
    exports.track = track;

    Object.defineProperty(exports, '__esModule', { value: true });

}));
//# sourceMappingURL=haya.umd.js.map
